...
{
	if (FMath::Square(Div) < SMALL_NUMBER)
	{
		return(0.0f);
	}
	return(X / Div);
...
FLinearColor UTGOR_Math::LerpColors(const FLinearColor& Dark, const FLinearColor& Medium, const FLinearColor& Light, uint8 Byte)
{
...
FLinearColor UTGOR_Math::SolidifyColor(const FLinearColor& Color)
{
	return(FLinearColor(Color.R, Color.G, Color.B, 1.0f));
}

FLinearColor UTGOR_Math::BuildColor(int32 DwColor)
{
	return(FColor(DwColor).ReinterpretAsLinear());
}
...
float UTGOR_Math::ProjectToBox(const FVector2D& Vector)
{
...
bool UTGOR_Math::ProjectToSlate(const FVector& Normal, const FVector& A, const FVector& B, const FVector& S, const FVector& T)
{
	const FVector AB = B - A;
	const FVector& Plane = UTGOR_Math::Normalize(AB ^ Normal);
...
bool UTGOR_Math::ComputeCircumcenter(const FVector& A, const FVector& B, const FVector& C, FVector& Out)
{
...
bool UTGOR_Math::ComputeCircumcenter2D(const FVector2D& A, const FVector2D& B, const FVector2D& C, FVector2D& Out)
{
	const float aa = (B - C).SizeSquared();
	const float bb = (C - A).SizeSquared();
	const float cc = (A - B).SizeSquared();
