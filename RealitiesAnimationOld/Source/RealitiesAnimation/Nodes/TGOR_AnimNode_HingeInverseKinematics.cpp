...
void FTGOR_AnimNode_HingeInverseKinematics::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	// TODO: Produces crash in packaged build
	//NewtonsMethod3x3(Config, Tail, 1.0f);
	Config = FVector::ZeroVector;
	FTransform Transform = ComputeEETransform(Tail, Config);
	float Energy = ComputeEEEnergy(Transform, Config);
	CachedConfig = Config;
...
FTransform FTGOR_AnimNode_HingeInverseKinematics::ComputeEETransform(const FTransform& Tail, const FVector& Config)
{
	...
	return Tail * CacheZ.Component;
}

float FTGOR_AnimNode_HingeInverseKinematics::ComputeEEEnergy(const FTransform& Transform, const FVector& Config)
{
	...
	return 0.5f * (ENERGY(X) + ENERGY(Y) + ENERGY(Z) + Error.SizeSquared()) + RotError * Rotational;
}


bool FTGOR_AnimNode_HingeInverseKinematics::NewtonsMethod3x3(FVector& Config, const FTransform& Tail, float Rate)
{
...
#define HESSIAN_DIAG(I) HESSIAN(I, I) + Stabiliser.I
		...
		for (int32 Bisect = 0; Bisect < MaxIterations; Bisect++)
		{
			Config -= Delta * Rate;

			Transform = ComputeEETransform(Tail, Config);
			const float Result = ComputeEEEnergy(Transform, Config);
			if (Result < Energy)
			{
				Energy = Result;
				Rate = Rate * 1.1f;
