...
bool UTGOR_Command::ParseContent(FTGOR_CommandArg& CommandArg, FString& Error, const FTGOR_CommandArgument& Argument, const FString& Arg)
{
	...
	TArray<UTGOR_CoreContent*> List = ContentManager->GetTListFromType<UTGOR_CoreContent>(Argument.String.Content);

	// See if resourceable exists
	for (UTGOR_CoreContent* Content : List)
	{
		if (Argument.String.AllowAbstract || !Content->IsAbstract())
		{
			for (const FString& String : Content->GetAliases())
			{
				...
				{
					CommandArg.String = String;
					CommandArg.Index = ContentManager->GetTIndex<UTGOR_CoreContent>(Content);
...
UTGOR_CoreContent* UTGOR_Command::FetchContent(const TMap<FString, FTGOR_CommandArg>& Args, const FString& Name)
{
	const FTGOR_CommandArg* Arg = Args.Find(Name);
	if (Arg)
	{
