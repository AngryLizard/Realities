...
float FTGOR_TrackerEntry::Count(const TArray<FTGOR_TrackerEntry>& Entries, const TArray<TSubclassOf<UTGOR_CoreContent>>& Filter, int32 Count) const
{
	...
	for (const TPair<UTGOR_CoreContent*, int32> Pair : Subs)
	{
		// Add up number of matches recursively
		int32 Match = 0;
		for (const TSubclassOf<UTGOR_CoreContent>& Class : Filter)
		{
...
void FTGOR_TrackerEntry::Put(TArray<FTGOR_TrackerEntry>& Entries, const TArray<UTGOR_CoreContent*>& Registry, int32 Index, float Delta)
{
	if (Registry.IsValidIndex(Index))
	{
		UTGOR_CoreContent* Content = Registry[Index];
		if (IsValid(Content))
		{
			Counter += Delta;
...
float FTGOR_TrackerInstance::Count(const TArray<TSubclassOf<UTGOR_CoreContent>>& Filter) const
{
...
{
	return Root.Count(Entries, TArray<TSubclassOf<UTGOR_CoreContent>>(), 0);
}

void FTGOR_TrackerInstance::Put(const TSet<UTGOR_CoreContent*>& Registry, float Delta)
{
	TArray<UTGOR_CoreContent*> List = Registry.Array();
	List.Remove(nullptr);
	List.Sort([](const UTGOR_CoreContent& A, const UTGOR_CoreContent& B) -> bool { return A.GetStorageIndex() < B.GetStorageIndex(); });
	Root.Put(Entries, List, 0, Delta);
}

void FTGOR_TrackerInstance::Put(float Delta)
{
	Root.Put(Entries, TArray<UTGOR_CoreContent*>(), 0, Delta);
}
