...
#include "Realities/Components/Creature/TGOR_ChildSkeletalMeshComponent.h"
#include "RealitiesUGC/Mod/TGOR_ContentManager.h"
#include "Realities/Base/TGOR_Singleton.h"

#include "Realities/Mod/Unlocks/Customisations/TGOR_Customisation.h"
#include "Realities/Mod/Unlocks/Bodyparts/TGOR_Bodypart.h"
#include "Realities/Mod/Unlocks/Colours/TGOR_Colour.h"
#include "Realities/Mod/Unlocks/Skins/TGOR_Skin.h"
...
void UTGOR_BodypartNode::UpdateAppearance()
{
	if (IsValid(CurrentMaster))
	{
		CurrentMaster->OnAppearanceChanged.Broadcast();
	}
}
...
void UTGOR_BodypartNode::AttachAndInitialise(bool OverrideMaster)
{
	...
	if (IsValid(Bodypart))
	{
		...
		if (Bodypart->FollowsMasterPose)
		{
			Bodypart->AttachMesh(Mesh.Mesh, CurrentMaster);
		}
...
void UTGOR_BodypartNode::CreateMesh()
{
	...
	MeshInstance.Mesh = NewObject<UTGOR_ChildSkeletalMeshComponent>(this);
...
void UTGOR_BodypartNode::ApplyInternalCustomisation()
{
	if (IsValid(Bodypart))
	{
		for (auto Pair : Mesh.CustomisationInstances)
		{
			for (UMaterialInstanceDynamic* MaterialInstance : Pair.Value.MaterialInstances)
			{
...
UTGOR_BodypartNode* UTGOR_BodypartNode::AddChildType(TSubclassOf<UTGOR_Bodypart> Class)
{
	if (IsValid(Bodypart))
	{
...
UTGOR_BodypartNode* UTGOR_BodypartNode::AddChildBodypart(UTGOR_Bodypart* Content)
{
	...
	if (IsValid(Bodypart))
	{
		for (const FTGOR_BodypartChild& Requirement : Bodypart->Children)
		{
			if (Requirement.Type == ETGOR_BodypartChildType::Required)
			{
				...
				UTGOR_BodypartNode* Node = GetChildFromType(Requirement.Class);
				if (!IsValid(Node))
				{
...
void UTGOR_BodypartNode::ConstructPath(TArray<FTGOR_BodypartInstance>& Path) const
{
