...
void UTGOR_UnlockComponent::RepNotifyPending(const FTGOR_KnowledgeCollection& Old)
{
	...
	while (Queue.Num() > 0)
	{
		...
		if (!Pending.Data.Contains(Knowledge))
		{
			...
			Queue.Append(Knowledge->Children);
		}
	}
...
bool UTGOR_UnlockComponent::CheckPendingKnowledges(const FTGOR_TrackerData& Trackers)
{
	...
	for (auto Itr = Pending.Data.CreateIterator(); Itr; ++Itr)
	{
		if (CheckKnowledge(Trackers, *Itr))
		{
			...
			Pending.Data.Append((*Itr)->Children);
...
bool UTGOR_UnlockComponent::ContainsUnlock(UTGOR_Content* Content) const
{
	for (UTGOR_Knowledge* Knowledge : Unlocked)
	{
		if (Knowledge->ContainsInsertion(Content))
		{
			return true;
		}
	}
...
bool UTGOR_UnlockComponent::ContainsUnlockFromType(TSubclassOf<UTGOR_Content> Type) const
{
	for (UTGOR_Knowledge* Knowledge : Unlocked)
	{
		if (Knowledge->ContainsInsertionFromType(Type))
		{
			return true;
		}
	}
...
TArray<UTGOR_Content*> UTGOR_UnlockComponent::FilterUnlock(TArray<UTGOR_Content*> List) const
{
	TArray<UTGOR_Content*> Out;
	for (UTGOR_Content* Content : List)
	{
		if (ContainsUnlock(Content))
		{
			Out.Emplace(Content);
		}
	}
...
UTGOR_Content* UTGOR_UnlockComponent::GetUnlockedFromType(TSubclassOf<UTGOR_Content> Type) const
{
	for (UTGOR_Knowledge* Knowledge : Unlocked)
	{
		UTGOR_Content* Content = Knowledge->GetInsertionFromType(Type);
		if (IsValid(Content))
		{
			return Content;
		}
...
TArray<UTGOR_Content*> UTGOR_UnlockComponent::GetUnlockedListFromType(TSubclassOf<UTGOR_Content> Type) const
{
	TArray<UTGOR_Content*> List;
	for (UTGOR_Knowledge* Knowledge : Unlocked)
	{
		List.Append(Knowledge->GetInsertionListFromType(Type));
	}
	return List;
}
