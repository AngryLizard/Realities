...
void UTGOR_CameraComponent::RegisterHandle(TScriptInterface<ITGOR_RegisterInterface> Register, UTGOR_Content* Content, TSubclassOf<UTGOR_Camera> Type, const FVector& Params, float Blend)
{
	if (*Type)
	{
		for (auto& Pair : Cameras)
...
void UTGOR_CameraComponent::UnregisterHandle(UTGOR_Content* Content, TSubclassOf<UTGOR_Camera> Type)
{
...
void UTGOR_CameraComponent::BuildModifiers(UTGOR_Content* Content)
{
	Handles.Empty();
	Cameras.Empty();
	if (IsValid(Content))
	{
		const TArray<UTGOR_Camera*> List = Content->GetIListFromType<UTGOR_Camera>();
...
void UTGOR_CameraComponent::TickCamera(float DeltaTime)
{
	SpringArm->SetWorldRotation(ViewRotation);
	...
	for (auto& Pair : Cameras)
	{
		...
		for (UTGOR_Content* Layer : Queue.Layers)
		{
...
void UTGOR_CameraComponent::BuildCameraQueues(TMap<UTGOR_Camera*, FTGOR_CameraQueue>& Queues)
{
	...
	for (auto Handle = Handles.CreateIterator(); Handle; ++Handle)
	{
		...
		else
		{
			...
			const TSet<UTGOR_Content*> ContentContext = Handle->Value.Register->Execute_GetActiveContent(Handle->Value.Register.GetObject());
...
void UTGOR_CameraComponent::TranslateCameraTowards(const FVector& WorldLocation, float Blend)
{
	RegisterHandle(nullptr, nullptr, UTGOR_FixCamera::StaticClass(), WorldLocation, Blend);
}

void UTGOR_CameraComponent::RotateCameraTowards(const FVector& WorldLocation, float Blend)
{
	RegisterHandle(nullptr, nullptr, UTGOR_LookatCamera::StaticClass(), WorldLocation, Blend);
}
