...
#include "Realities/Actors/Dimension/TGOR_SkyboxActor.h"
	...
	: Super()
{
	...
	CosmeticTypes.Add(AAtmosphericFog::StaticClass());
...
void UTGOR_Dimension::SetCullDistance(AActor* Actor) const
{
	...
	{
		UPrimitiveComponent* Mesh = Cast<UPrimitiveComponent>(Component);
		if (Mesh && Mesh->CachedMaxDrawDistance < SMALL_NUMBER || CullDistance < Mesh->CachedMaxDrawDistance)
		{
			Mesh->SetCullDistance(CullDistance);
		}
	}
