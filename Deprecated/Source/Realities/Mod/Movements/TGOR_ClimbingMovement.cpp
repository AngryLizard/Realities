...
void UTGOR_ClimbingMovement::QueryInputVector(const UTGOR_MovementComponent* Component, FVector& OutInput, FVector& OutView) const
{
	...
	const FQuat& MovementBase = Component->GetComponentQuat();

	OutInput = MovementBase * Remapping;
	OutView = InputRotation.GetAxisX();
}

void UTGOR_ClimbingMovement::Tick(UTGOR_MovementComponent* Component, const FTGOR_MovementTick& Tick, const FTGOR_MovementSpace& Space, const FTGOR_MovementExternal& External, bool Replay, FTGOR_MovementOutput& Out)
{
	...
	else
	{
		...
		for (const FHitResult& HitResult : HitResults)
		{
			...
			if (Input)
			{
				...
				const float FinalMaximumSpeed = Bendness * Bendness * MaximumSpeed * Frame.Agility;
			...
			Out.SetBaseFrom(HitResult.GetActor(), HitResult.BoneName);
