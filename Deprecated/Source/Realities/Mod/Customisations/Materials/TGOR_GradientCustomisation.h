...
*/
UCLASS()
class REALITIES_API UTGOR_GradientCustomisation : public UTGOR_MaterialCustomisation
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintPure, Category = "TGOR Customisation", Meta = (Keywords = "C++"))
		void ToPayload(TArray<uint8>& Payload, UTGOR_Colour* Content, float ValueX, float ValueY) const;

	/** Set payload */
	UFUNCTION(BlueprintPure, Category = "TGOR Customisation", Meta = (Keywords = "C++"))
		void FromPayload(const TArray<uint8>& Payload, UTGOR_Colour*& Content, float& ValueX, float& ValueY);
