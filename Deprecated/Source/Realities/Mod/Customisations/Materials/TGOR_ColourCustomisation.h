...
#include "TGOR_ColourCustomisation.generated.h"

/**
*
*/
UCLASS()
class REALITIES_API UTGOR_ColourCustomisation : public UTGOR_MaterialCustomisation
{
	GENERATED_BODY()

public:
	UTGOR_ColourCustomisation();
	...
	UFUNCTION(BlueprintPure, Category = "TGOR Customisation", Meta = (Keywords = "C++"))
		void ToPayload(TArray<uint8>& Payload, const FColor& Colour) const;

	/** Set payload */
	UFUNCTION(BlueprintPure, Category = "TGOR Customisation", Meta = (Keywords = "C++"))
		void FromPayload(const TArray<uint8>& Payload, FColor& Colour);

	/** Colour parameter name for material */
