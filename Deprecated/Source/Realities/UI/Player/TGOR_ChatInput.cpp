...
{
	HistoryPointer = 0;
	SystemColour = FLinearColor(1.0f, 1.0f, 0.0f, 1.0f);
...
FLinearColor UTGOR_ChatInput::CreateMessageColour(const FTGOR_ChatMessageInstance& Message)
{
	switch (Message.Domain)
	{
		case ETGOR_ChatDomainEnumeration::System:
		{
			// Get colour for system calls
			return(SystemColour);
		}
		case ETGOR_ChatDomainEnumeration::Public:
		{
			...
			{
				return(Message.Channel->GetChannelColour());
			}
			break;
		}
...
{
	const FLinearColor Colour = CreateMessageColour(Message);
	switch (Message.Domain)
	{
		case ETGOR_ChatDomainEnumeration::System:
		{
			if (IsValid(Message.Command))
			{
				...
				ChangeDomain(Message.Domain, Colour, CreateCommandString(Message.Command));
		...
		case ETGOR_ChatDomainEnumeration::Public:
		{
			if (IsValid(Message.Channel))
			{
				...
				ChangeDomain(Message.Domain, Colour, CreateChannelString(Message.Channel));
				ChangeContent(Message.Message);
			}
			break;
		}
			...
			if (IsValid(Controller))
			{
				...
				for (const FTGOR_UserInfo& Info : Controller->GetOnlineUsers())
				{
					if (Info.Properties.Serial == Message.UserSerial)
					{
						...
						ChangeDomain(Message.Domain, Colour, CreateUserString(Info.Properties));
						ChangeContent(Message.Message);
						break;
					}
				}
...
void UTGOR_ChatInput::FindState(const FString& Receiver, const FString& Content)
{
	...
	// Get channel and use a defined colour mapping to privileges
	ETGOR_FetchEnumeration State;
	...
	{
		CurrentPrefix = L"/";
		// Get channel and use its colour
		...
		{
			// Get player and use their colour
