...
public:
	...
	void Prepare(UTGOR_ActionComponent* Component, const FTGOR_SlotIndex& Index);

	/** Performs the current frame in the current state */
	bool Wait(UTGOR_ActionComponent* Component, float Deltatime);
	...
	void Replicate(UTGOR_ActionComponent* Component, bool Force);

	/** Updates server storage with given storage class from client. */
	void Integrate(UTGOR_ActionStorage* Storage);
