...
void UTGOR_EventModule::BuildModule(const UTGOR_Storage* Parent)
{
	...
	Events = Content->GetIListFromType<UTGOR_Event>();
...
void UTGOR_EventModule::Tick(UTGOR_DimensionComponent* Owner, float DeltaSeconds)
{
	Super::Tick(Owner, DeltaSeconds);

	// This assumes replication happens before tick
	Prompted = Activated;
}
