...
void UTGOR_InputModule::BuildModule(const UTGOR_Storage* Parent)
{
	...
	UTGOR_Content* Content = Parent->GetOwner();
	Inputs = Content->GetIListFromType<UTGOR_Input>();
...
bool UTGOR_InputModule::IsActive(TSubclassOf<UTGOR_Input> Input) const
{
	for (int32 Index = 0; Index < Inputs.Num(); Index++)
	{
		UTGOR_Input* Content = Inputs[Index];
...
bool UTGOR_InputModule::HasTurnedOn(TSubclassOf<UTGOR_Input> Input) const
{
	for (int32 Index = 0; Index < Inputs.Num(); Index++)
	{
		UTGOR_Input* Content = Inputs[Index];
...
bool UTGOR_InputModule::HasTurnedOff(TSubclassOf<UTGOR_Input> Input) const
{
	for (int32 Index = 0; Index < Inputs.Num(); Index++)
	{
		UTGOR_Input* Content = Inputs[Index];
