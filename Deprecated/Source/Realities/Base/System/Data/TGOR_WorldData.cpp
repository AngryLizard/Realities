...
UTGOR_IdentityComponent* UTGOR_WorldData::GetIdentity(const FTGOR_DimensionIdentifier& Identifier, ETGOR_FetchEnumeration& Branches) const
{
	...
	{
		const FTGOR_DimensionDataInstance& Instance = Dimensions[Identifier.DimensionIdentifier];
...
FTGOR_DimensionIdentifier UTGOR_WorldData::GetIdentifier(AActor* Actor, ETGOR_FetchEnumeration& Branches) const
{
	...
	if (IsValid(Actor))
	{
		...
		if (IsValid(Identity))
		{
			...
			Identifier.DimensionIdentifier = Identity->GetDimensionIdentifier(State);
...
void UTGOR_WorldData::AssembleContentManager(UWorld* World)
{
	PersistentWorld = World;

	ContentManagerAssembled = true;

	ATGOR_WorldSettings* Settings = Cast<ATGOR_WorldSettings>(World->GetWorldSettings());
	if (IsValid(Settings))
	{
		RealityHeight = Settings->RealityMaxHeight;
		MiniDimensionHeight = Settings->MiniDimensionsMaxHeight;
	}
}
...
void UTGOR_WorldData::AssemblePersistent(UTGOR_Dimension* Dimension, bool IsServer)
{
	...
	if (!HasFinishedLoading(PersistentIdentifier))
	{
		...
		{
			// Assign dimension
			FTGOR_DimensionDataInstance& Instance = Dimensions[PersistentIdentifier];
			...
			ATGOR_GameState* GameState = Cast<ATGOR_GameState>(GetWorld()->GetGameState());
			if (IsValid(GameState) && GameState->HasAuthority())
			{
				GameState->DimensionUpdateState.Dimensions.Add(PersistentIdentifier, Instance.Location);
			}
...
bool UTGOR_WorldData::AddIdentifier(UTGOR_Dimension* Content, const FName& Identifier)
{
	...
	FTGOR_DimensionDataInstance NewInstance;
...
bool UTGOR_WorldData::SetIdentifier(UTGOR_Dimension* Content, const FName& Identifier)
{
	...
	{
		FTGOR_DimensionDataInstance& Instance = Dimensions[Identifier];
...
{
	Branches = ETGOR_FetchEnumeration::Empty;
	FTGOR_DimensionDataInstance* Dimension = Dimensions.Find(Identifier);
...
{
	const FTGOR_DimensionDataInstance* Dimension = Dimensions.Find(Identifier);
...
bool UTGOR_WorldData::ProgressLoadDimension(const FName& Identifier, bool IsServer)
{
	...
	FTGOR_DimensionDataInstance& Dimension = Dimensions[Identifier];
	...
	if (!IsValid(World))
	{
		ERRET("World not set", Error, false)
	}
	...
	switch (Dimension.Stage)
	{
		case ETGOR_DimensionLoadingStage::Unloaded:
		{
			...
			if (!IsServer)
			{
				FVector* Ptr = GameState->DimensionUpdateState.Dimensions.Find(Identifier);
				if (!Ptr)
				{
					break;
				}
		...
		case ETGOR_DimensionLoadingStage::Loaded:
		{
			...
			if (IsValid(Streaming) && Streaming->IsLevelVisible())
			{
				...
				{
					GameState->DimensionUpdateState.Dimensions.Add(Identifier, Dimension.Location);
				}
	...
	switch (Dimension.Stage)
	{
		case ETGOR_DimensionLoadingStage::Cleanup:
		{
			if (IsServer)
			{
				...
				// Unload from all clients
				UTGOR_UserData* UserData = Singleton->GetUserData();
				UserData->ForEachController([&](ATGOR_OnlineController* OnlineController) -> bool
					{
						OnlineController->RemoveDimensionUpdate(Identifier);
						return true;
					});
			}
		...
		case ETGOR_DimensionLoadingStage::UnloadingClients:
		{
			if (IsServer)
			{
				...
				UserData->ForEachController([&](ATGOR_OnlineController* OnlineController) -> bool
					{
						if (!OnlineController->IsLocalController())
						{
							// Check whether removal is done on all clients or timed out
							const FTGOR_Time* Timestamp = OnlineController->PendingDeletions.Find(Identifier);
			...
			if (IsValid(Dimension.Dimension) && Dimension.Dimension->LevelStreaming)
			{
				...
				if (IsServer)
				{
					GameState->DimensionUpdateState.Dimensions.Remove(Identifier);
				}
...
void UTGOR_WorldData::UnloadAllRealities()
{
	...
	{
		FTGOR_DimensionDataInstance& Instance = Iterator->Value;
