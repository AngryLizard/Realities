...
void ATGOR_VolumeAllocationTest::DrawVolumes(FLinearColor BranchColour, FLinearColor LeafColour, FLinearColor VolumeColour, float Duration, float Thicc)
{
	// Initialse transform and colours
	const FTransform Transform = GetActorTransform();
	const FColor BColour = BranchColour.ToFColor(false);
	const FColor LColour = LeafColour.ToFColor(false);
	const FColor VColour = VolumeColour.ToFColor(false);
	...
	_spatialTree.ForEach([=](const FVector& Center, const FVector& Extend, bool Leaf)
	{
		...
		DrawDebugBox(GetWorld(), WorldCenter, WorldExtend, Leaf ? LColour : BColour, false, Duration, 0x00, Thicc);
	});
	...
	for (const FTGOR_VolumeAllocation Volume : Volumes)
	{
		...
		DrawDebugBox(GetWorld(), WorldCenter, WorldExtend, VColour, false, Duration, 0x00, Thicc);
	}
}