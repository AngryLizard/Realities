...
struct REALITIESANIMATION_API FTGORRigUnit_Deform : public FTGORRigUnit_Mutable
{
	 ...
	 */
	UPROPERTY(meta = (Input, DetailsOnly))
		FVector ConeAimAxis = FVector::ForwardVector;
	...
	UPROPERTY(meta = (Input, ExpandByDefault))
		FRigElementKey AbsBaseKey = FRigElementKey(FName(), ERigElementType::Bone);
...
struct REALITIESANIMATION_API FTGORRigUnit_DeformChain : public FTGORRigUnit_DeformRetarget
{
	 ...
	 */
	UPROPERTY(meta = (Input, ExpandByDefault))
		FRigElementKey AbsTipKey = FRigElementKey(FName(), ERigElementType::Bone);
...
struct REALITIESANIMATION_API FTGORRigUnit_DeformTranslate : public FTGORRigUnit_DeformPivot
{
	 ...
	 */
	UPROPERTY(meta = (Input, ExpandByDefault))
		FRigElementKey AbsPivotKey = FRigElementKey(FName(), ERigElementType::Bone);

	/**
	 * The Abstractor bone moving around the pivot
	 */
	UPROPERTY(meta = (Input, ExpandByDefault))
		FRigElementKey AbsTargetKey = FRigElementKey(FName(), ERigElementType::Bone);
...
struct REALITIESANIMATION_API FTGORRigUnit_DeformInitial : public FTGORRigUnit_DeformPivot
{
	 ...
	 */
	UPROPERTY(meta = (Input, ExpandByDefault))
		FRigElementKey AbsRefKey = FRigElementKey(FName(), ERigElementType::Bone);
...
struct REALITIESANIMATION_API FTGORRigUnit_DeformRoot : public FTGORRigUnit_DeformPivot
{
	...
	/**
	 * Top bone on abstractor. The root is oriented between tail and head.
	 */
	UPROPERTY(meta = (Input, ExpandByDefault))
		FRigElementKey AbsTopKey = FRigElementKey(FName(), ERigElementType::Bone);
 ...
 */
USTRUCT(meta = (DisplayName = "Deform Anchor", Keywords = "TGOR,Abstractor", PrototypeName = "DeformAnchor", NodeColor = "1.0 0.44 0.0"))
struct REALITIESANIMATION_API FTGORRigUnit_DeformAnchor : public FTGORRigUnit_DeformPivot
{
	GENERATED_BODY()

		FTGORRigUnit_DeformAnchor() {}
	...
	UPROPERTY(Transient)
		FCachedRigElement AbsRefCache;
	UPROPERTY(Transient)
		FCachedRigElement ObjLeftCache;
	UPROPERTY(Transient)
		FCachedRigElement ObjRightCache;
