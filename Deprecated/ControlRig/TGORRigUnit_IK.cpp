...
FTGORRigUnit_HingeChainLength_Execute()
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_RIGUNIT()
	...
	else
	{
		...
		else
		{
			...
			const FTransform InitialTarget = Hierarchy->GetInitialGlobalTransform(Chain[2]);
			Lengths.X = (InitialUpper.GetLocation() - InitialLower.GetLocation()).Size() * Customisation.X;
			Lengths.Y = (InitialLower.GetLocation() - InitialTarget.GetLocation()).Size() * Customisation.Y;

			// Rescale to fit length
			ChainMaxLength = Lengths.X + Lengths.Y;
...
FString FTGORRigUnit_HingeChainLength::ProcessPinLabelForInjection(const FString& InLabel) const
{
	FString Formula;
	return FString::Printf(TEXT("%s: TODO"), *InLabel);
}
...
FTGORRigUnit_HingeIK_Execute()
{
	...
	else
	{
		...
		else
		{
			...
			float Kath = 0.0f;
			float Height = 0.0f;
			ComputeTriangle(Lengths.X, Lengths.Y, ObjectiveNorm, Height, Kath);
...
FTGORRigUnit_DigitigradeChainLength_Execute()
{
	DECLARE_SCOPE_HIERARCHICAL_COUNTER_RIGUNIT()
	...
	else
	{
		...
		else
		{
			...
			// Rescale to fit length
			ChainMaxLength = Lengths.X + Lengths.Y + Lengths.Z * Digitness;
			if (!FMath::IsNearlyZero(ChainMaxLength))
			{
				const float TargetRatio = TargetLength / ChainMaxLength;
...
FString FTGORRigUnit_DigitigradeChainLength::ProcessPinLabelForInjection(const FString& InLabel) const
{
	FString Formula;
	return FString::Printf(TEXT("%s: TODO"), *InLabel);
}
...
FTGORRigUnit_DigitigradeIK_Execute()
{
	...
	else
	{
		...
		{
			const FVector FootForwardTarget = Objective.TransformVectorNoScale(ObjectiveForwardAxis);
			const FVector FootUpTarget = Objective.TransformVectorNoScale(ObjectiveUpAxis);
			...
			const FVector AnkleDirection = UTGOR_Math::Normalize(FVector::VectorPlaneProject(Direction + FootForwardTarget, ObjectiveNormal));
			const FVector PlantiAnkle = Objective.GetLocation() - FootForwardTarget * Lengths.Z * FootPivot;
			...
			const float HeelHeight = (AnkleLocation - Objective.GetLocation()) | FootUpTarget;
			const float SoleLength = FMath::Sqrt((Lengths.Z * Lengths.Z) - (HeelHeight * HeelHeight));
			const FVector Heel = AnkleLocation - FootUpTarget * HeelHeight;
			const FVector FootLocation = Heel + FootForwardTarget * SoleLength;
			...
			Foot.SetRotation(FTGORRigUnit_RotateToward::ComputeHeadingRotation(EffectorForwardAxis, FootForwardTarget, EffectorUpAxis, FootUpTarget));
			Foot.SetLocation(FootLocation);
			Hierarchy->SetGlobalTransform(Chain[3], Foot, PropagateToChildren != ETGOR_Propagation::Off);

			if (DebugSettings.bEnabled)
			{
				Context.DrawInterface->DrawLine(FTransform::Identity, FootLocation, FootLocation + FootForwardTarget * 20.0f, FLinearColor::Red, DebugSettings.Scale * 0.5f);
				Context.DrawInterface->DrawLine(FTransform::Identity, FootLocation, FootLocation + FootUpTarget * 20.0f, FLinearColor::Blue, DebugSettings.Scale * 0.5f);
