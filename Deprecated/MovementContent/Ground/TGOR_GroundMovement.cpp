...
#include "Realities/CreatureSystem/Customisation/TGOR_ModularSkeletalMeshComponent.h"
	...
	: Super()
{
	...
	MaximumLegStrength = 1'600'000.0f;
	LegDropOffMultiplier = 0.2f;
	...
	OverrideInsertions.Add(UTGOR_RagdollMovement::StaticClass());
}
...
void UTGOR_GroundMovement::QueryInputVector(const UTGOR_MovementComponent* Component, FVector& OutInput, FVector& OutView) const
{
	...
	if (Size >= SMALL_NUMBER)
	{
		...
		const float Input = FMath::Min(1.0f, InputStrength * InputSize * InputMultiplier);
...
void UTGOR_GroundMovement::Tick(UTGOR_MovementComponent* Component, const FTGOR_MovementTick& Tick, const FTGOR_MovementSpace& Space, const FTGOR_MovementExternal& External, FTGOR_MovementOutput& Out)
{
	...
	// Set base output
	Out.Parent = Component->FindReparentToComponent(Contact.Component, Contact.Name);
	...
	GetStandingForce(Component, Tick, Space, External, Contact.FrameNormal, Contact, Stretch, Out);
	GetStandingTorque(Component, Tick, Space, External, Contact.FrameNormal, SpeedRatio, Out);
	...
	// Call on animation
	if (CrouchMultiplier > SMALL_NUMBER)
	{
...
float UTGOR_GroundMovement::GetInputForce(UTGOR_MovementComponent* Component, const FTGOR_MovementTick& Tick, const FTGOR_MovementSpace& Space, const FTGOR_MovementExternal& External, const FTGOR_MovementContact& Contact, const FTGOR_MovementRepel& Repel, float GroundRatio, FTGOR_MovementOutput& Out) const
{
	...
	Component->GetDampingForce(Tick, Contact.FrameVelocity, BrakeCoefficient * Frame.Strength, Out);
	Component->GetDampingTorque(Tick, Space.RelativeAngularVelocity, AngularDamping * Frame.Strength, Out);
	return 0.0f;
}
...
void UTGOR_GroundMovement::GetStandingForce(UTGOR_MovementComponent* Component, const FTGOR_MovementTick& Tick, const FTGOR_MovementSpace& Space, const FTGOR_MovementExternal& External, const FVector& Normal, const FTGOR_MovementContact& Contact, float Stretch, FTGOR_MovementOutput& Out) const
{
	...
	Component->GetDampingForce(Tick, Space.RelativeLinearVelocity.ProjectOnTo(Normal), DampingCoeff, Out);
	...
	if ((Contact.GroundDistance < LegLength && Relative > 10.0f) || Relative > SMALL_NUMBER)
	{
